...
Base.BaseWidget {
    ...
    tooltipText: {
        if (!Services.NetworkService.connected) return "Disconnected"
    ...
    Row {
        ...
        Components.BarIcon {
            ...
            icon: {
                if (!Services.NetworkService.connected) return Services.NetworkService.getDisconnectedIcon()
                if (Services.NetworkService.isWifi) return Services.NetworkService.getSignalIcon(Services.NetworkService.signalStrength)
