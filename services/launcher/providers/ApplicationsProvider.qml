...
QtObject {
    ...
    function _matches(app: QtObject, q: string): bool {
        if (app.name && app.name.toLowerCase().includes(q)) return true;
        if (app.genericName && app.genericName.toLowerCase().includes(q)) return true;
        if (app.comment && app.comment.toLowerCase().includes(q)) return true;
        if (app.keywords) {
